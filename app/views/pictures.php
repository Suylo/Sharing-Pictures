		...
		<div class="textContent">
			<div class="inlineRow">
                ...
                <?php if (self::$UIDofUserLogged == self::$UID) { ?>
					<form method="post">
						<button type="submit" name="delete_picture" class="input_icons" title="Supprimer la photo"><i class="bi bi-x-circle"></i></button>
					</form>
	            <?php } ?>
			</div>
			...
			<div class="comment">
				<div>
                    ...
                    <?php if (isset($_POST["editCaption"]) == false && self::$UIDofUserLogged == self::$UID) { ?>
						<form method="post">
							<button type="submit" name="editCaption" class="input_icons" title="Modifier la description"><i class="bi bi-pencil"></i></button>
						</form>
                    <?php } else if (self::$UIDofUserLogged == self::$UID && isset($_POST["editCaption"]) == true) { ?>
						<form method="post">
							...
							<button type="submit" name="saveEditedCaption" class="input_icons" title="Sauvegarder"><i class="bi bi-save"></i></button>
						</form>
                    <?php } ?>
				</div>
	            ...
	            <?php if (self::$listOfComments != null) {
	                foreach (self::$listOfComments as $key => $val) {
			            ...
			            <?php if ($Username->getUserEmail() == UserAuth::getInstance()->getMailLoggedOn()) {?>
							<p style="margin-top: 10px;">
				                <div style="display: flex; align-items: center">
					                ...
					                <form method="post">
						                <button type="submit" name="delete_comment" title="Supprimer ce commentaire" class="input_icons" value="<?= $val["commentID"] ?>"><i class="bi bi-x"></i></button>
					                </form>
				                </div>
							</p>
		                <?php } else { ?>
